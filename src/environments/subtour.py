...
from solvers.tsp import TSPSolver, CALLBACK_NAME
from solvers.callbacks.state_extractor.subtour import SubtourStateExtractor
from config import EnvConfig


class SubtourEnv(BaseCutEnv):
        ...
        self.observation_space = spaces.Dict(
            {
                ...
                "ori_node_feature": spaces.Box(
                    ...
                    shape=(self.init_config.instance_size, self.init_config.ori_node_dim),
                ),
    ...
    def reset(self, instance_path=None, steps=""):
        ...
        while True:
            ...
            self.problem = TSPProblem(instance_path)
            if len(self.problem.graph.nodes) == self.init_config.instance_size:
                break

        print("Processing instance", instance_path)

        self.solver = TSPSolver(problem=self.problem)
        self.state_extractor = SubtourStateExtractor(self.solver.separator, self.config)
        ...
        self.userSubtour.set_attribute(
            self.solver.separator, **user_cb_kwargs
        )
